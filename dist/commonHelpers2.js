import{l as a,f as c,m as l,c as N,s as g,a as x}from"./assets/exercises-right-part-filter-730a3265.js";import"./assets/vendor-295897c2.js";const q=document.querySelector(".wrap-button"),L=document.querySelector(".muscles-list"),H=document.querySelector(".pagination-numbers"),k=document.querySelector(".title-exercises");q.addEventListener("click",M);document.addEventListener("DOMContentLoaded",()=>{S({filter:"Muscles",page:1,limit:12})});function C(){var t=window.scrollY||window.pageYOffset||document.documentElement.scrollTop,e=Math.max(document.body.scrollHeight,document.documentElement.scrollHeight,document.body.offsetHeight,document.documentElement.offsetHeight,document.body.clientHeight,document.documentElement.clientHeight),s=window.innerHeight||document.documentElement.clientHeight||document.body.clientHeight;return t+s>=e}async function M(t){t.preventDefault(),k.innerHTML="Exercises";const{target:e,currentTarget:s}=t;if(e.nodeName!=="BUTTON")return;const i=e.dataset.name;e.classList.contains("btn-filter")&&[...s.children].forEach(o=>{o.firstElementChild.classList.remove("btn-filter-active")}),e.classList.add("btn-filter-active");const r={filter:i,page:1,limit:12};L.innerHTML="",S(r)}async function S(t){document.querySelector(".filter-list-js").classList.remove("exercises_list"),document.querySelector(".form-js").classList.add("hidden-form");try{a.open();const e=await c.getByFilterName(t);a.close();const s=e.results;if(!e||s.length===0){l.error("Sorry, we didn't find anything according to your request.");return}L.insertAdjacentHTML("beforeend",f(s)),window.addEventListener("scroll",function(){C()&&(console.log("Сторінка завантажилась до кінця!"),f(s))});const{totalPages:i}=e,r=JSON.stringify({totalPages:i,categoryName:t.filter});localStorage.setItem("infoRequest",r),document.querySelector(".filter-list-js").classList.add("muscles-list"),H.innerHTML="",h(i,t),u(1)}catch(e){console.log(e.message)}}function f(t){return t.map(({filter:e,name:s,imgURL:i})=>{let r=e.toLocaleLowerCase().replaceAll(" ","");return r==="bodyparts"&&(r="bodypart"),`
